...
<template>
    <div class="relative h-full">
        <div class="flex items-center justify-between mb-6">
            ...
            <button @click="abrirModal(null)" class="bg-amber-500 text-[#0d1b2a] font-['Barlow_Condensed'] text-sm font-bold tracking-wider uppercase px-5 py-2.5 rounded-lg border-none cursor-pointer inline-flex items-center gap-2 transition-all hover:bg-amber-400 hover:-translate-y-px shadow-sm">
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-amber-500/5 dark:bg-amber-500/10 border-b border-amber-500/20">
                            ...
                            <th class="px-4 py-2.5 text-right font-['Barlow_Condensed'] text-[11px] font-bold tracking-[0.12em] uppercase text-amber-600 dark:text-amber-500">Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-if="cargando">
                            <td colspan="5" class="px-4 py-6 text-center text-slate-500 text-sm font-medium">Cargando peajes desde la base de datos...</td>
                        </tr>
                        <tr v-else-if="peajes.length === 0">
                            <td colspan="5" class="px-4 py-8 text-center text-slate-500 text-sm">No hay peajes registrados aún. Haz clic en "Nuevo peaje".</td>
                        </tr>
                        <tr v-else v-for="peaje in peajes" :key="peaje.id" class="hover:bg-black/5 dark:hover:bg-white/5 transition-colors border-b border-slate-100 dark:border-white/5">
                            ...
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold tracking-wider uppercase transition-all bg-emerald-100 dark:bg-emerald-500/15 text-emerald-700 dark:text-emerald-400">
                                    <span class="w-1.5 h-1.5 rounded-full inline-block bg-emerald-400 shadow-[0_0_6px_#34d399]"></span> Activo
                                </span>
                            </td>
                            <td class="px-4 py-3 text-right">
                                <button @click="abrirModal(peaje)" class="p-1.5 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-500/20 rounded transition-colors cursor-pointer border-none bg-transparent inline-flex items-center justify-center">
        ...
        <div v-if="mostrarModal" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 dark:bg-[#0a1628]/80 backdrop-blur-sm transition-opacity p-4">
            <div class="w-full max-w-lg bg-white dark:bg-[#0d1b2a] border border-slate-200 dark:border-white/10 shadow-2xl overflow-hidden relative">
                ...
                <div class="px-6 py-4 border-b border-slate-100 dark:border-white/5 flex items-center justify-between">
                    <h3 class="font-['Barlow_Condensed'] text-[20px] font-bold text-slate-900 dark:text-slate-100 tracking-wide m-0">
