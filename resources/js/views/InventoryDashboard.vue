...
const formNuevoInsumo = ref({
    name: '', category: '', unit_measure: '', alert_threshold: 10, toll_id: ''
})
...
const registrarIngresoOP = async () => {
    ...
    try {
        ...
        toast.success('Orden de Provisión registrada exitosamente. Stock actualizado.')
        cerrarModalOP()
...
const registrarNuevoInsumo = async () => {
    ...
    try {
        ...
        toast.success('Nuevo insumo agregado al catálogo del peaje.')
        cerrarModalNuevoInsumo()
        await cargarInventario()
    } catch (error) {
        toast.error('Error al crear el insumo. Verifique los datos.')
    } finally {
        guardando.value = false
    }
}
...
onMounted(() => {
    cargarPeajes() // Cargamos los peajes primero
    cargarInventario()
})
</script>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50 dark:bg-white/5 border-b border-slate-200 dark:border-white/10">
                            <th class="p-4 font-['Barlow_Condensed'] text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Peaje / Sucursal</th>
                    ...
                    <tbody v-if="cargando">
                        <tr><td colspan="6" class="p-8 text-center text-slate-400 text-sm">Cargando inventario...</td></tr>
                    </tbody>
                    <tbody v-else-if="inventario.length === 0">
                        <tr><td colspan="6" class="p-8 text-center text-slate-400 text-sm">El catálogo está vacío. Registre un nuevo insumo para comenzar.</td></tr>
                    </tbody>
                    <tbody v-else>
                        <tr v-for="item in inventario" :key="item.id" class="border-b border-slate-100 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                            <td class="p-4">
                                <span class="text-[11px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest bg-indigo-50 dark:bg-indigo-500/10 px-2 py-1 rounded">
                                    {{ item.toll ? item.toll.name : 'Sin asignar' }}
                                </span>
                            </td>
                            <td class="p-4"><span class="font-bold text-slate-800 dark:text-slate-200 text-sm">{{ item.name }}</span></td>
                            <td class="p-4"><span class="text-xs px-2.5 py-1 rounded-full bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-300 font-medium">{{ item.category }}</span></td>
                            <td class="p-4 text-right">
                                <span class="font-mono text-base font-bold text-slate-900 dark:text-amber-400">{{ item.current_stock }}</span>
                                <span class="text-xs text-slate-500 dark:text-slate-400 ml-1 uppercase">{{ item.unit_measure }}</span>
                            </td>
                            <td class="p-4">
                                <span v-if="parseFloat(item.current_stock) <= parseFloat(item.alert_threshold)" class="inline-flex items-center gap-1.5 text-xs font-bold text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-500/10 px-2 py-1 rounded uppercase tracking-wider">
                                    <span class="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></span> Stock Bajo
                                </span>
                                <span v-else class="inline-flex items-center gap-1.5 text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider">
        ...
        <div v-if="mostrarModalOP" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div class="bg-white dark:bg-[#0d1b2a] w-full max-w-md shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden">
                ...
                <form @submit.prevent="registrarIngresoOP" class="p-6 space-y-4">
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">Material y Peaje a Ingresar</label>
                    ...
                    <div>
                        ...
                        <textarea v-model="formOP.description" rows="2" class="w-full bg-slate-50 dark:bg-black/20 border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2.5 text-slate-900 dark:text-white text-sm outline-none focus:border-amber-500/50 resize-none" placeholder="Detalles del proveedor..."></textarea>
                    </div>
        ...
        <div v-if="mostrarModalNuevoInsumo" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div class="bg-white dark:bg-[#0d1b2a] w-full max-w-md shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden">
                ...
                <form @submit.prevent="registrarNuevoInsumo" class="p-6 space-y-4">
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">Peaje / Sucursal Asignada</label>
                        <select v-model="formNuevoInsumo.toll_id" required class="w-full bg-slate-50 dark:bg-black/20 border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2.5 text-slate-900 dark:text-white text-sm outline-none focus:border-amber-500/50">
        ...
        <div v-if="mostrarModalEditar" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div class="bg-white dark:bg-[#0d1b2a] w-full max-w-md shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden">
                ...
                <form @submit.prevent="guardarEdicionInsumo" class="p-6 space-y-4">
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">Cambiar Peaje Asignado</label>
                        <select v-model="formEditarInsumo.toll_id" required class="w-full bg-slate-50 dark:bg-black/20 border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2.5 text-slate-900 dark:text-white text-sm outline-none focus:border-indigo-500/50">
                            <option v-for="peaje in peajes" :key="peaje.id" :value="peaje.id">{{ peaje.name }}</option>
                        </select>
                    </div>
