...
const imagenesParaLightbox = computed(() => {
    return evidenciasSeleccionadas.value.filter(archivo => esImagen(archivo))
})

const abrirLightbox = (url) => {
...
<template>
    <div class="h-full flex flex-col relative space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-1 space-y-4">
                <div class="bg-white dark:bg-[#0d1b2a] border border-slate-200 dark:border-white/10 p-5 shadow-sm ">
                    <h3
                        class="font-['Barlow_Condensed'] text-lg font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wide mb-4 border-b border-slate-100 dark:border-white/5 pb-2">
                        Stock Disponible (Bacheo)
                    </h3>

                    <div v-if="!formulario.toll_id"
                        class="text-sm text-amber-600 dark:text-amber-400 font-medium p-3 bg-amber-50 dark:bg-amber-500/10 rounded border border-amber-200 dark:border-amber-500/20">
                    ...
                    <div v-else-if="materialConsumido.length === 0" class="text-sm text-slate-500 italic">Esta sucursal
                        no posee insumos de bacheo registrados.</div>
                    <div v-else class="space-y-4">
                        <div v-for="item in materialConsumido" :key="item.inventory_item_id"
                            class="flex justify-between items-center bg-slate-50 dark:bg-white/5 p-3 rounded-lg border border-slate-200 dark:border-white/5">
                            <div>
                                <span class="block text-sm font-bold text-slate-700 dark:text-slate-300">{{ item.name
                                }}</span>
                                <span class="text-[11px] text-slate-500 uppercase tracking-widest">{{ item.unit_measure
                                }}</span>
                            </div>
            ...
            <div class="lg:col-span-2">
                <div class="bg-white dark:bg-[#0d1b2a] border border-slate-200 dark:border-white/10 p-6 shadow-sm ">
                    <h3
                        class="font-['Barlow_Condensed'] text-lg font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wide mb-5 border-b border-slate-100 dark:border-white/5 pb-2">
                    ...
                    <form @submit.prevent="registrarTrabajoBacheo" class="space-y-5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div class="md:col-span-2">
                                <label
                                    class="block text-[11px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest mb-1.5">
                                    1. Sucursal / Peaje afectado
                                </label>
                                <select v-model="formulario.toll_id" required
                                    class="w-full bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 rounded-lg px-4 py-2.5 text-slate-900 dark:text-white text-sm outline-none focus:border-indigo-500">
                            ...
                            <div class="md:col-span-2">
                                <label
                                    class="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">2.
                                    Ubicación / Referencia</label>
                            ...
                            <div class="md:col-span-2">
                                <label
                                    class="block text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">3.
                                    Detalle del trabajo realizado</label>
                                <textarea v-model="formulario.description" rows="3" required
                                    :disabled="!formulario.toll_id"
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input id="bacheo-file" type="file" multiple @change="procesarArchivo"
                                    accept="image/*,.pdf" class="hidden" />
                                <label for="bacheo-file"
                                    :class="{ 'opacity-50 cursor-not-allowed': !formulario.toll_id, 'cursor-pointer hover:border-indigo-500/50 hover:bg-slate-100 dark:hover:bg-white/5': formulario.toll_id }"
                                    ...
                                    <div
                                        class="bg-indigo-100 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 p-2 rounded-full">
                        ...
                        <div v-if="materialConsumido.length > 0"
                            class="mt-6 bg-slate-50 dark:bg-white/5 p-4 rounded-lg border border-slate-200 dark:border-white/10">
                            <label
                                class="block text-[12px] font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest mb-3">
                                5. Material Consumido en este arreglo
                            </label>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div v-for="(mat, index) in materialConsumido" :key="index" class="flex flex-col">
                                    <span class="text-[11px] font-semibold text-slate-600 dark:text-slate-400 mb-1">{{
                                        mat.name }} ({{ mat.unit_measure }})</span>
                                    <input v-model="mat.quantity" type="number" step="0.01" min="0"
                                        :max="mat.current_stock"
        ...
        <div
            class="bg-white dark:bg-[#0d1b2a] border border-slate-200 dark:border-white/10 overflow-hidden shadow-sm mt-6  flex-1 flex flex-col">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-slate-200 dark:border-white/10 bg-white dark:bg-black/20">
                            <th
                                class="px-6 py-3 font-['Barlow_Condensed'] text-[11px] font-bold text-slate-500 uppercase tracking-widest">
                                Fecha</th>
                            <th
                                class="px-6 py-3 font-['Barlow_Condensed'] text-[11px] font-bold text-slate-500 uppercase tracking-widest">
                                Ubicación</th>
                            <th
                                ...
                                Detalle</th>
                            <th
                                class="px-6 py-3 font-['Barlow_Condensed'] text-[11px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">
                                Evidencia</th>
                            <th
                                class="px-6 py-3 font-['Barlow_Condensed'] text-[11px] font-bold text-slate-500 uppercase tracking-widest text-right">
                                Acciones</th>
                        </tr>
                    </thead>
                    <tbody v-if="cargandoHistorial">
                        <tr>
                            <td colspan="5" class="p-6 text-center text-slate-400 text-sm">Cargando historial...</td>
                        </tr>
                    </tbody>
                    <tbody v-else-if="historialBacheo.length === 0">
                        <tr>
                            <td colspan="5" class="p-6 text-center text-slate-400 text-sm">No hay registros en esta
                                vista.</td>
                        </tr>
                    </tbody>
                    <tbody v-else>
                        <tr v-for="trabajo in historialBacheo" :key="trabajo.id"
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-3">
                                    <template v-if="vistaActual === 'activos'">
                                        <button @click="abrirModalEditar(trabajo)"
                                            class="text-[11px] font-bold uppercase tracking-widest text-indigo-500 hover:text-indigo-700 bg-transparent border-none cursor-pointer p-0">Editar</button>
                                        <button @click="archivarTrabajo(trabajo.id)"
                                            class="text-[11px] font-bold uppercase tracking-widest text-slate-500 hover:text-slate-700 bg-transparent border-none cursor-pointer p-0">Archivar</button>
                                    </template>
                                    <template v-else>
                                        <button @click="restaurarTrabajo(trabajo.id)"
                                            class="text-[11px] font-bold uppercase tracking-widest text-emerald-500 hover:text-emerald-700 bg-transparent border-none cursor-pointer p-0">Restaurar</button>
                                    </template>
                                </div>
                            </td>
                        </tr>
            ...
            <div v-if="paginacion.total > 0 && !cargandoHistorial"
                class="px-6 py-4 flex items-center justify-between bg-slate-50 dark:bg-white/5 border-t border-slate-200 dark:border-white/10 mt-auto">
                <span class="text-xs text-slate-500 dark:text-slate-400">
                    Página <strong>{{ paginacion.current_page }}</strong> de <strong>{{ paginacion.last_page }}</strong>
                </span>
                <div class="flex items-center gap-2">
                    <button @click="cargarHistorial(paginacion.current_page - 1)"
                        :disabled="paginacion.current_page === 1"
                        class="px-3 py-1 text-xs font-bold text-slate-600 border border-slate-300 rounded hover:bg-white disabled:opacity-50 cursor-pointer bg-transparent">Anterior</button>
                    <button @click="cargarHistorial(paginacion.current_page + 1)"
                        :disabled="paginacion.current_page === paginacion.last_page"
                        class="px-3 py-1 text-xs font-bold text-slate-600 border border-slate-300 rounded hover:bg-white disabled:opacity-50 cursor-pointer bg-transparent">Siguiente</button>
                </div>
            </div>
        </div>
        ...
        <div v-if="mostrarModalEditar"
            ...
            <div
                class="bg-white dark:bg-[#0d1b2a] w-full max-w-lg shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden ">
                <div
                ...
                <form @submit.prevent="guardarEdicion" class="p-6 space-y-4">
                    <div>
                        <label
                            class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">Ubicación
                            / Referencia</label>
                        <input v-model="formEditar.title" type="text" required
                    ...
                    <div>
                        <label
                            class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">Detalle
                            del trabajo realizado</label>
                        <textarea v-model="formEditar.description" rows="3" required
                            class="w-full bg-slate-50 dark:bg-black/20 border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2.5 text-slate-900 dark:text-white text-sm outline-none focus:border-indigo-500/50 resize-none"></textarea>
                    </div>

                    <div class="mb-2">
                        <label
                            class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-1.5">
                            Anexar nuevas fotos (Las anteriores se conservan)
                        </label>
                        <input id="edit-bacheo-file" type="file" multiple @change="procesarArchivo"
                            accept="image/*,.pdf" class="hidden" />
                        <label for="edit-bacheo-file"
                            class="w-full flex flex-col items-center justify-center gap-2 bg-slate-50 dark:bg-[#0a1628] border border-dashed border-slate-300 dark:border-white/20 px-3 py-4 text-xs font-semibold text-slate-500 cursor-pointer rounded-lg hover:border-indigo-500/50 transition-all">
                            <span>Seleccionar archivos para subir...</span>
                        </label>
                        <div v-if="archivosAdjuntos.length > 0" class="flex flex-col gap-1.5 mt-3">
                            <div v-for="(archivo, i) in archivosAdjuntos" :key="i"
                                class="bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 rounded-md px-3 py-2 flex items-center justify-between">
                                <span
                                    class="text-[11px] font-medium text-indigo-700 dark:text-indigo-300 truncate pr-2">{{
                                        archivo.name }}</span>
                                <button type="button" @click="quitarArchivo(i)"
                                    class="text-[10px] font-bold text-red-500 bg-transparent border-none cursor-pointer uppercase">Quitar</button>
                            </div>
                        </div>
                    </div>

                    <div
                        class="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/10 p-3 rounded">
                        <strong>Nota:</strong> La edición de materiales consumidos no está permitida por políticas de
                        auditoría
                        de inventario.
                    </div>
                    ...
                    <div class="pt-4 flex justify-end gap-3 border-t border-slate-100 dark:border-white/5">
                        ...
                        <button type="submit" :disabled="guardandoEdicion"
                            class="bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2 rounded-lg text-sm font-bold uppercase tracking-wider font-['Barlow_Condensed'] transition-all border-none cursor-pointer disabled:opacity-50 shadow-sm">
        ...
        <div v-if="mostrarVisor"
            ...
            <div
                class="w-full max-w-4xl bg-white dark:bg-[#0d1b2a] border border-slate-200 dark:border-white/10 shadow-2xl flex flex-col max-h-[90vh]  overflow-hidden">
                <div
                    ...
                    <button @click="cerrarVisor"
                        class="text-slate-500 hover:text-red-500 bg-transparent border-none cursor-pointer"><svg
                ...
                <div
                    ...
                    <div v-for="(archivo, index) in evidenciasSeleccionadas" :key="index"
                        ...
                        <div v-if="esImagen(archivo)"
                            class="aspect-video w-full bg-slate-200 dark:bg-black/50 relative">
                            ...
                            <div
                                ...
                                <div
                                    class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                    <button type="button" @click="abrirLightbox(archivo)"
                                        class="bg-amber-500 text-black px-4 py-2 rounded-lg font-['Barlow_Condensed'] font-bold tracking-wider uppercase text-sm border-none cursor-pointer hover:bg-amber-400 hover:scale-105 transition-transform">
                                        Ampliar Foto
                                    </button>
                                </div>
    ...
    <ImageLightbox :mostrar="mostrarLightbox" :imagenes="imagenesParaLightbox" :imagenInicial="imagenSeleccionada"
