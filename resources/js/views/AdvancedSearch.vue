...
const limpiarFiltros = () => {
    filtros.value = { texto: '', toll_id: '', incident_type: '', date_from: '', date_to: '' }
    resultados.value = []
...
onMounted(() => {
    ...
    if (route.query.q) {
        filtros.value.texto = route.query.q
        ejecutarBusqueda()
    }
})
</script>
...
<template>
    <div>
        <div class="mb-6">
            ...
            <p class="text-[13px] text-slate-500 dark:text-slate-400 m-0">Filtros cruzados para auditoría de pesajes y
                sucesos</p>
        </div>

        <div
            class="bg-white dark:bg-[#0d1b2a] border border-slate-200 dark:border-white/10  shadow-sm mb-6 transition-colors">
            ...
            <form @submit.prevent="ejecutarBusqueda" class="p-5">
                ...
                <div class="mb-4">
                    <label
                        class="font-['Barlow_Condensed'] text-[11px] font-bold tracking-[0.12em] uppercase text-slate-500 dark:text-slate-400 block mb-1.5">Términos
                        de búsqueda</label>
                    <div class="relative">
                        <input type="text" v-model="filtros.texto"
                            placeholder="Buscar detalles, observaciones, o tipos de sucesos..."
                            class="w-full bg-slate-50 dark:bg-white/5 border border-slate-300 dark:border-white/10 rounded-lg pl-10 pr-4 py-2 text-slate-900 dark:text-white text-sm outline-none focus:border-amber-500/50 transition-colors" />
                        <svg class="absolute left-3.5 top-2.5 text-slate-400 w-4 h-4" viewBox="0 0 24 24" fill="none"
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-5">
                    <div>
                        <label
                            class="font-['Barlow_Condensed'] text-[11px] font-bold tracking-[0.12em] uppercase text-slate-500 dark:text-slate-400 block mb-1.5">Estación</label>
                    ...
                    <div>
                        <label
                            class="font-['Barlow_Condensed'] text-[11px] font-bold tracking-[0.12em] uppercase text-slate-500 dark:text-slate-400 block mb-1.5">Clasificación</label>
                        <select v-model="filtros.incident_type"
                            class="w-full bg-slate-50 dark:bg-white/5 border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-slate-900 dark:text-white text-xs outline-none focus:border-amber-500/50">
                            ...
                            <option value="accidente_vial">Accidentes</option>
                    ...
                    <div>
                        <label
                            class="font-['Barlow_Condensed'] text-[11px] font-bold tracking-[0.12em] uppercase text-slate-500 dark:text-slate-400 block mb-1.5">Desde
                            la fecha</label>
                        <input type="date" v-model="filtros.date_from"
                            class="w-full bg-slate-50 dark:bg-white/5 border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-slate-900 dark:text-white text-xs outline-none focus:border-amber-500/50 [color-scheme:light] dark:[color-scheme:dark]" />
                    </div>

                    <div>
                        <label
                            class="font-['Barlow_Condensed'] text-[11px] font-bold tracking-[0.12em] uppercase text-slate-500 dark:text-slate-400 block mb-1.5">Hasta
                            la fecha</label>
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-slate-100 dark:border-white/5">
                    <button type="button" @click="limpiarFiltros"
                        class="px-5 py-2 rounded-lg text-xs font-bold text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/5 transition-colors border-none bg-transparent font-['Barlow_Condensed'] uppercase tracking-wider cursor-pointer">
                        Limpiar Filtros
                    </button>
                    <button type="submit" :disabled="cargando"
                        class="bg-amber-500 text-[#0d1b2a] font-['Barlow_Condensed'] text-xs font-bold tracking-wider uppercase px-6 py-2 rounded-lg border-none cursor-pointer inline-flex items-center transition-all hover:bg-amber-400 disabled:opacity-50">
        ...
        <div v-if="busquedaRealizada"
            class="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10  overflow-hidden shadow-sm transition-colors">
            <div
                class="bg-slate-50 dark:bg-white/5 border-b border-slate-200 dark:border-white/10 px-5 py-3 flex justify-between items-center">
                <div class="flex items-center gap-3">
                    <span
                        class="font-['Barlow_Condensed'] font-bold text-[15px] text-slate-900 dark:text-slate-100">Resultados
                        obtenidos</span>
                    <span
                        class="text-xs font-semibold text-amber-600 dark:text-amber-500 bg-amber-50 dark:bg-amber-500/10 px-2.5 py-1 rounded-md">{{
                            resultados.length }} registros</span>
                </div>

                <button v-if="resultados.length > 0" @click="exportarExcel"
                    class="flex items-center gap-2 bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-200 dark:hover:bg-emerald-500/30 px-3 py-1.5 rounded-md text-[11px] font-bold tracking-wider uppercase transition-colors border-none cursor-pointer">
            ...
            <div class="overflow-x-auto">
                <table class="w-full border-collapse">
                    ...
                    <tbody>
                        <tr v-if="cargando">
                            <td colspan="4" class="px-4 py-10 text-center text-slate-500 text-sm">Procesando consulta...
                            </td>
                        </tr>
                        <tr v-else-if="resultados.length === 0">
                            <td colspan="4" class="px-4 py-10 text-center text-slate-500 text-sm">No se encontraron
                                registros que coincidan con los filtros aplicados.</td>
                        </tr>
                        <tr v-else v-for="suceso in resultados" :key="suceso.id"
                            class="hover:bg-black/5 dark:hover:bg-white/5 border-b border-slate-100 dark:border-white/5 group">
                            <td class="px-4 py-3 text-[13px] text-slate-600 dark:text-slate-400 font-medium">{{
                                formatearFecha(suceso.created_at) }}</td>
                            <td class="px-4 py-3 font-semibold text-slate-900 dark:text-slate-100">{{ suceso.toll ?
                                suceso.toll.name : 'N/A' }}</td>
                            <td class="px-4 py-3"><span
                                    class="inline-flex px-2 py-1 rounded-md text-[11px] font-bold uppercase bg-slate-100 dark:bg-white/10 text-slate-700 dark:text-slate-300">{{
                                        traducirTipo(suceso.incident_type) }}</span></td>
                            <td class="px-4 py-3">
                ...
                <div v-if="totalRegistros > 0"
                    ...
                    <div class="flex gap-2">
                        <button type="button" @click="ejecutarBusqueda(paginaActual - 1)" :disabled="paginaActual === 1"
                        ...
                        <button type="button" @click="ejecutarBusqueda(paginaActual + 1)"
            ...
            <div
                ...
                <div
                    ...
                    <button @click="cerrarDetalles"
                        class="text-slate-400 hover:text-red-500 bg-transparent border-none cursor-pointer transition-colors p-1"><svg
                ...
                <div
                    ...
                    <button @click="cerrarDetalles"
                        class="px-5 py-2 rounded-lg text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-white/10 transition-colors border-none bg-slate-200/50 dark:bg-white/5 cursor-pointer font-['Barlow_Condensed'] uppercase tracking-wider">Cerrar</button>
                </div>
            </div>
        </div>
