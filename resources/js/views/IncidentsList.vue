    ...
    <div class="h-full relative">
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex gap-3">
                ...
                <router-link to="/panel/sucesos/nuevo" class="bg-amber-500 text-[#0d1b2a] font-['Barlow_Condensed'] text-sm font-bold tracking-wider uppercase px-5 py-2.5 rounded-lg border-none cursor-pointer inline-flex items-center gap-2 transition-all hover:bg-amber-400 hover:-translate-y-px">
                    + Reportar Suceso
                </router-link>
            </div>
        </div>
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-slate-50 dark:bg-white/5 border-b border-slate-200 dark:border-white/10">
                            ...
                            <th class="px-4 py-3 text-left font-['Barlow_Condensed'] text-[11px] font-bold tracking-[0.12em] uppercase text-slate-500 dark:text-slate-400">Datos Operativos</th>
                            <th class="px-4 py-3 text-left font-['Barlow_Condensed'] text-[11px] font-bold tracking-[0.12em] uppercase text-amber-600 dark:text-amber-500">Evidencia</th>
                    ...
                    <tbody>
                        <tr v-if="cargando">
                            <td colspan="6" class="px-4 py-8 text-center text-slate-500 text-sm">Cargando base de datos...</td>
                        </tr>
                        <tr v-else-if="sucesos.length === 0">
                            <td colspan="6" class="px-4 py-8 text-center text-slate-500 text-sm">No se encontraron sucesos en esta categoría.</td>
                        </tr>
                        ...
                        <tr v-else v-for="suceso in sucesos" :key="suceso.id" class="hover:bg-black/5 dark:hover:bg-white/5 transition-colors border-b border-slate-100 dark:border-white/5 group">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <router-link v-if="!verArchivados" :to="`/panel/sucesos/editar/${suceso.id}`" class="p-1.5 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-500/20 rounded transition-colors cursor-pointer">
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                                    </router-link>

                                    <button v-if="!verArchivados" @click="confirmarArchivado(suceso)" class="p-1.5 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-500/20 rounded transition-colors cursor-pointer border-none bg-transparent">
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><polyline points="21 8 21 21 3 21 3 8"></polyline><rect x="1" y="3" width="22" height="5"></rect><line x1="10" y1="12" x2="14" y2="12"></line></svg>
                                    </button>
        ...
        <div v-if="mostrarModalArchivar" class="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/90 backdrop-blur-sm p-4">
            <div class="w-full max-w-sm bg-white dark:bg-[#0a1628] rounded-xl shadow-2xl overflow-hidden border border-slate-200 dark:border-white/10 text-center p-6">
                <div class="w-12 h-12 bg-red-100 dark:bg-red-500/20 rounded-full flex items-center justify-center mx-auto mb-4 text-red-600 dark:text-red-400">
                ...
                <h3 class="font-['Barlow_Condensed'] text-[20px] font-bold text-slate-900 dark:text-slate-100 mb-2">¿Archivar registro?</h3>
                <p class="text-sm text-slate-500 dark:text-slate-400 mb-6">El suceso #{{ sucesoAArchivar?.id }} será movido a la tabla de archivados. Pasará a estado de Solo Lectura y no podrá ser modificado en el futuro.</p>
                
                <div class="flex gap-3 justify-center">
                    <button @click="mostrarModalArchivar = false" class="px-5 py-2 rounded-lg text-xs font-bold text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 transition-colors border-none cursor-pointer font-['Barlow_Condensed'] uppercase tracking-wider">Cancelar</button>
                    <button @click="ejecutarArchivado" class="bg-red-500 text-white font-['Barlow_Condensed'] text-xs font-bold tracking-wider uppercase px-5 py-2 rounded-lg border-none cursor-pointer hover:bg-red-600 transition-colors">Confirmar Archivo</button>
                </div>
            </div>
        </div>

        <div v-if="mostrarGaleria" class="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/90 backdrop-blur-sm transition-opacity p-4">
            <div class="w-full max-w-4xl bg-white dark:bg-[#0a1628] shadow-2xl overflow-hidden flex flex-col max-h-full border border-slate-200 dark:border-white/10">
                <div class="px-6 py-4 border-b border-slate-200 dark:border-white/10 flex items-center justify-between bg-slate-50 dark:bg-[#0d1b2a]">
                    <div>
                        <h3 class="font-['Barlow_Condensed'] text-[22px] font-extrabold text-slate-900 dark:text-slate-100 tracking-wide m-0">Portafolio de Evidencia</h3>
                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium">
                            <span class="text-amber-600 dark:text-amber-500 font-bold">{{ traducirTipo(sucesoActivo?.incident_type) }}</span>
                            · Reportado el {{ formatearFecha(sucesoActivo?.created_at) }}
                        </p>
                    </div>
                    <button @click="cerrarGaleria" class="text-slate-400 hover:text-red-500 bg-transparent border-none cursor-pointer transition-colors p-1">
                ...
                <div class="p-6 overflow-y-auto bg-slate-100 dark:bg-[#0a1628] flex-1">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div v-for="(ruta, nombreCampo) in mediosActivos" :key="nombreCampo" class="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 overflow-hidden shadow-sm flex flex-col">
                            <div class="px-4 py-2.5 bg-slate-50 dark:bg-[#0d1b2a] border-b border-slate-200 dark:border-white/10 flex items-center justify-between">
                                <span class="text-[11px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest font-['Barlow_Condensed']">
                                    {{ nombreCampo.replace('_', ' ') }}
                                </span>
                            </div>
                            <div class="aspect-video bg-slate-200 dark:bg-black/40 flex items-center justify-center relative group overflow-hidden">
                                <img v-if="esImagen(ruta)" :src="ruta" class="w-full h-full object-cover bg-black/5" />
                                ...
                                <div v-else class="flex flex-col items-center gap-3 p-6 text-center">
                                    <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="text-slate-400"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline></svg>
                                    <span class="text-sm text-slate-500">Documento no previsualizable</span>
                                </div>
                                <a :href="ruta" target="_blank" class="absolute inset-0 bg-slate-900/60 backdrop-blur-[2px] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity text-white no-underline font-['Barlow_Condensed'] tracking-widest text-sm font-bold uppercase z-10">
        ...
        <div v-if="mostrarDetalles" class="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/90 backdrop-blur-sm transition-opacity p-4">
            <div class="w-full max-w-md bg-white dark:bg-[#0a1628] shadow-2xl overflow-hidden flex flex-col border border-slate-200 dark:border-white/10">
                ...
                <div class="px-6 py-4 border-b border-slate-200 dark:border-white/10 flex items-center justify-between bg-slate-50 dark:bg-[#0d1b2a]">
                    ...
                    <button @click="cerrarDetalles" class="text-slate-400 hover:text-red-500 bg-transparent border-none cursor-pointer transition-colors p-1">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                    </button>
                </div>
